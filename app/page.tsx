      ...
      <main className="flex-1 flex flex-col h-full border-r border-neutral-800">
        ...
        <div className="p-4 border-b border-neutral-800 flex items-center justify-between bg-black">
          ...
          <div className="flex bg-neutral-900 rounded-full p-1 border border-neutral-800">
            <button onClick={() => setMode('chaos')} className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${mode === 'chaos' ? 'bg-red-600 text-white shadow-lg' : 'text-neutral-500 hover:text-white'}`}>
              <Flame size={14} /> Chaos
            </button>
            <button onClick={() => setMode('crypto')} className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${mode === 'crypto' ? 'bg-blue-600 text-white shadow-lg' : 'text-neutral-500 hover:text-white'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-neutral-600 space-y-2">
              <p className="text-sm">{mode === 'chaos' ? "Chaos mode active. Prepare to be roasted." : "Crypto mode active. Paste a token address for live data."}</p>
            </div>
          )}
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`group relative max-w-[85%] p-4 rounded-2xl border ${m.role === 'user' ? 'bg-neutral-800 border-neutral-700 text-white' : mode === 'chaos' ? 'bg-red-950/20 border-red-900/50 text-neutral-200' : 'bg-blue-950/20 border-blue-900/50 text-neutral-200'}`}>
                {m.type === 'image' ? <img src={m.text} className="rounded-xl max-w-full" alt="ai gen" /> : <p className="text-sm whitespace-pre-wrap leading-relaxed">{m.text}</p>}
          ...
          {loading && <div className="text-xs text-neutral-500 animate-pulse">Chet is scanning the blockchain...</div>}
        </div>
        ...
        <div className="p-4 bg-black border-t border-neutral-800">
          <div className="flex gap-2 mb-3 overflow-x-auto pb-1 scrollbar-hide">
            ...
            <button onClick={() => { setMode('crypto'); setInput("Scan this token: "); }} className="flex items-center gap-1 text-xs bg-neutral-900 border border-neutral-800 px-3 py-1.5 rounded-full text-neutral-400 hover:text-white hover:bg-neutral-800 transition-colors whitespace-nowrap">
              <ShieldAlert size={12} /> Scam Radar
            </button>
          </div>

          <form onSubmit={(e) => { e.preventDefault(); sendMessage(input); }} className="relative">
            {/* FIXED TEXT COLOR HERE (text-white placeholder:text-neutral-500) */}
            <input 
              value={input} 
              onChange={(e) => setInput(e.target.value)} 
              className={`w-full bg-neutral-900 text-white placeholder:text-neutral-500 py-4 px-6 pr-14 rounded-full focus:outline-none focus:ring-1 transition-all ${mode === 'chaos' ? 'focus:ring-red-600' : 'focus:ring-blue-600'}`} 
