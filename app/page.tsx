...
// MAGIC FORMATTER: Turns **text** into actual Bold HTML
...
export default function Home() {
  ...
  const [messages, setMessages] = useState<{ role: "user" | "chet"; text: string; type?: 'text' | 'image' }[]>([
    { role: "chet", text: "Are you retarded? What is your name?" }
  ]);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) =>[...prev, { role: "chet", text: "My brain is buffering. Either you broke me, or my dev forgot to pay the OpenAI bill 💀" }]);
            ...
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 fade-in duration-300`}>
              <div className={`group relative max-w-[85%] p-4 rounded-2xl border shadow-sm ${
                m.role === 'user' 
                  ...
                  : mode === 'chaos' 
                    ? 'bg-red-50 dark:bg-red-950/20 border-red-100 dark:border-red-900/50 text-gray-800 dark:text-neutral-300' 
                    : 'bg-blue-50 dark:bg-blue-950/20 border-blue-100 dark:border-blue-900/50 text-gray-800 dark:text-neutral-300'
              }`}>
                {m.type === 'image' ? (
                  <img src={m.text} className="rounded-xl max-w-full shadow-md" alt="ai gen" />
                ) : (
                  <p className="text-sm whitespace-pre-wrap leading-relaxed">
                    {/* WE APPLY THE MAGIC FORMATTER HERE */}
                    {formatText(m.text)}
                  </p>
                )}
        ...
        <div className="p-4 bg-white dark:bg-[#050505] border-t border-gray-200 dark:border-neutral-800 transition-colors duration-300">
          ...
          <form onSubmit={(e) => { e.preventDefault(); sendMessage(input); }} className="relative">
            <input 
              ...
              placeholder={mode === 'chaos' ? "Type something stupid..." : "Paste a contract address..."} 
            />
